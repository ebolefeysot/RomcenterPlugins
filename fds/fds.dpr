...
//Datas definitions
// .fds format : Calculate the crc without header
// else calculate the crc of the full file.
...
const
  C_PlugInName = 'Fds crc calculator'; //full name of plug in
  C_Author = 'Eric Bole-Feysot'; //your name
  C_Version = '1.4'; //version of plug in
  C_WebPage = 'www.romcenter.com'; //home page of plug in
  C_Email = 'help@romcenter.com'; //Email of plug in support
  C_Description = 'Fds crc calculator. Skip the fds file header to calculate the crc32.  Support .fds format.';

  C_BUFFER_SIZE = 33554432; // 32MB
  C_ROMMINSIZE = $0; //min size of the core rom

  FORMAT_RAW='';
  FORMAT_1='.fds';
  FORMAT_2='.fds'; //shifted header
...
// 00000000h: 01 2A 4E 49 4E 54 45 4E 44 4F 2D 48 56 43 2A A4 ; .*NINTENDO-HVC*�
//
// type 2: header shifted by $10
// 00000000h: 46 44 53 1A 02 00 00 00 00 00 00 00 00 00 00 00 ; FDS.............
// 00000010h: 01 2A 4E 49 4E 54 45 4E 44 4F 2D 48 56 43 2A    ; .*NINTENDO-HVC*
//
...
var
  ...
  length_1:integer;
begin
    length_1 := length(TEXT_1);

    //Create dynamic array to store file content
    GetMem(FileBuf, LENGTH_1+1);
    try

      //detect type 1 header
      stream.Seek(OFFSET_1, soFromBeginning);
      stream.Read(FileBuf^, LENGTH_1);
      FileBuf[LENGTH_1] := char($00);
      if uppercase(string(StrPas(FileBuf))) = TEXT_1 then begin
        result := FORMAT_1;
        header_size := HEADERSIZE_1;
        rom_size := Stream.Size - header_size;
        exit;
      end;

      //detect type 2 header
      stream.Seek(OFFSET_2, soFromBeginning); //shift header
      stream.Read(FileBuf^, LENGTH_1);
      FileBuf[LENGTH_1] := char($00);
      if uppercase(string(StrPas(FileBuf))) = TEXT_1 then begin
        result := FORMAT_2;
        header_size := HEADERSIZE_2;
        comment := 'Header shifted by $10';
        rom_size := Stream.Size - header_size;
        exit;
      end;
