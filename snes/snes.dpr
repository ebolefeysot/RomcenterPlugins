...
//Datas definitions
// formats recognized: .mgd, .smc, .fig
// no header: mgd
// 512B header with AA BB 04 at offset 8-10 : SMC
// Other 512B header : FIG
//_________________

resourcestring
  TXT_TOOSMALL = 'Too small for a snes rom';
  TXT_BADSIZE = 'Bad file size';

//These datas defines infos about this plug in. Theey are loaded by romcenter.
//Use of this plug in depends of these datas;
const
  C_PlugInName = 'Snes crc calculator'; //full name of plug in
  ...
  C_Description = 'Snes crc calculator. If header is detected, it is skipped ' +
    'in the crc calculation. Check format mgd, smc and fig';
...
begin
  ...
  try
    try
      ...
      size := Stream.Size;
      //a snes rom is > 8KB
      if (Stream.Size < 8192) then begin
        ErrorMessage := pchar(TXT_TOOSMALL);
        exit;
      end;

      //a snes rom is a multiple of 8KB
      //the header size is the rom size modulo 8KB
      RomHeaderSize := Stream.Size mod 8192;

      //snes roms have a header of 512 B, or no header
      if (RomHeaderSize <> 0) and (RomHeaderSize <> 512) then begin
        ErrorMessage := pchar(TXT_BADSIZE);
        exit;
      end;

      //calculate format
      if RomHeaderSize = 0 then Format := '.mgd'
      else begin
        p := Stream.Memory;
        INC(p,8);

        //check for '$AABB04' in the rom, offset 8
        format := '.fig';
        if (p)^ = $AA then begin
          inc(p);
          if (p)^ = $BB then begin
            inc(p);
            if (p)^ = $04 then Format := '.smc';
          end;
        end;
      end;
      ...
      CalcCRC32(Stream.Memory, RomHeaderSize, Stream.Size, CRCvalue);
    except
...
begin
  ...
  //calculate crc32 of the rom core
