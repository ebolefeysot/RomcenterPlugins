...
//
// (C) Copyright 2018 Eric Bole-Feysot
// All Rights Reserved.
//
...
const
  C_PlugInName = 'Gameboy crc calculator'; //full name of plug in
  C_Author = 'Eric Bole-Feysot'; //your name
  C_Version = '1.1'; //version of plug in
  C_WebPage = 'www.romcenter.com'; //home page of plug in
  C_Email = 'help@romcenter.com'; //Email of plug in support
  C_Description = 'Gameboy crc calculator. Skip the Gameboy file header to calculate the crc32. Support .gb, .gbc and .sgb format.';

  C_BUFFER_SIZE = 33554432; // 32MB
  C_ROMMINSIZE = $0; //min size of the core rom

  FORMAT_RAW='';
  FORMAT_GB='.gb'; //0x146 = 0x00
  FORMAT_GBC='.gbc'; //0x143 = 0x80 or 0x143 = 0xC0
  FORMAT_SGB='.sgb'; //0x146 = 0x03 and 0x14B = 0x33
...
begin
    ...
    //find the header size ($0/$100)
    stream.Seek(HEADERSIZE_1 + OFFSET_1, soFromBeginning);
    stream.ReadBuffer(b[0],4);

    values := LPDWORD(b);

    if LPDWORD(b)^ = logo then begin
      header_size := HEADERSIZE_1;
    end
    else begin
      stream.Seek(HEADERSIZE_2 + OFFSET_1, soFromBeginning);
      stream.ReadBuffer(b[0],4);
      if LPDWORD(b)^ = logo then begin
    ...
    try
      stream.Seek(header_size + OFFSET_2, soFromBeginning);
      stream.Read(b[0], 1);
      if (b[0] = $80) or (b[0] = $C0) then begin
        //gbc/sgb
        stream.Seek(header_size + OFFSET_3, soFromBeginning);
        stream.Read(b[0], 1);
        if b[0] = $03 then begin
          //sgb
          result := FORMAT_SGB;
          exit;
        end;
